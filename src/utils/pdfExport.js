...
function strWidth(str, bold, size) {
    let w = 0;
    for (const ch of String(str || '')) { w += (WIDTHS_REGULAR[ch] ?? WIDTHS_REGULAR.default); }
    return (w / 1000) * size;
}
...
export async function generatePDF(_element, resumeData, template = 'classic') {
    ...
    const drawTextRight = (str, rx, py, font, size, col) => { const w = strWidth(str, false, size); drawText(str, rx - w, py, font, size, col); };
    ...
    const drawSectionTitle = (label) => {
        ...
        if (template === 'minimal') {
            ...
            // Minimal uses a grid-like layout, we'll shift y back up slightly for the content
            y += 0; 
        } else {
            drawText(label.toUpperCase(), ML, y, 'F2', 9, accent);
            y -= 4; ops.push('0.88 0.85 0.83 RG', '0.3 w', `${ML} ${y} m`, `${PW-MR} ${y} l`, 'S'); y -= 10;
        }
    };
    ...
    if (template === 'modern') {
       ...
       y = PH - 45;
       drawText((personal.name || 'YOUR NAME').toUpperCase(), ML, y, 'F2', 26, C_WHITE);
       y -= 26;
       drawText(personal.title || '', ML, y, 'F1', 11, accent);
       
       y = PH - 110 - 25; // Contact bar
       fillRect(0, y, PW, 22, '0.94 0.90 0.87');
       const contact = [personal.email, personal.phone, personal.location].filter(Boolean).join('  |  ');
       drawText(contact, ML, y + 7, 'F1', 8.5, C_MID);
       y -= 25;
    } else if (template === 'minimal') {
       y = PH - 60;
       drawText((personal.name || 'YOUR NAME').toUpperCase(), ML, y, 'F2', 24, accent);
       y -= 22;
       drawText(personal.title || '', ML, y, 'F1', 11, C_MID);
       y -= 14;
       const contact = [personal.email, personal.phone, personal.location].filter(Boolean).join(' · ');
       drawText(contact, ML, y, 'F1', 9, C_LIGHT);
       y -= 10; hline(y, 0.3, '0.88 0.85 0.83'); y -= 24;
    } else {
       y = PH - 60;
       drawText((personal.name || 'YOUR NAME').toUpperCase(), PW/2 - strWidth(personal.name, true, 26)/2, y, 'F2', 26, C_BLACK);
       y -= 26;
       const contact = [personal.email, personal.phone, personal.location].filter(Boolean).join(' · ');
       drawText(contact, PW/2 - strWidth(contact, false, 9)/2, y, 'F1', 9, C_MID);
       y -= 12; hline(y, 1); y -= 20;
    }
    ...
    for (const key of sectionOrder) {
        ...
        if (key === 'summary') {
            if (personal.summary) {
                ...
                drawWrapped(personal.summary, (template === 'minimal' ? ML + 110 : ML), (template === 'minimal' ? CW - 110 : CW), 'F1', 10, C_BLACK, 14);
                y -= 8;
            }
            continue;
        }
        ...
        drawSectionTitle(label);
        const startX = (template === 'minimal' ? ML + 110 : ML);
        const colW = (template === 'minimal' ? CW - 110 : CW);
        
        if (['skills', 'interests', 'languages'].includes(key)) {
            const list = items.map(it => typeof it === 'string' ? it : (it.name + (it.level ? ` (${it.level})` : ''))).join('  ·  ');
            drawWrapped(list, startX, colW, 'F1', 10, C_BLACK, 14);
            y -= 8;
        } else {
            for (const item of items) {
                ...
                if (title) {
                    drawText(title, startX, y, 'F2', 10.5, C_BLACK);
                    if (date) drawTextRight(date, PW - MR, y, 'F1', 9, C_LIGHT);
                    y -= 13;
                }
                const meta = [item.company, item.organization, item.institution, item.location, item.issuer].filter(Boolean).join(' · ');
                if (meta) { drawText(meta, startX, y, 'F1', 9.5, C_MID); y -= 12; }
                if (item.url) { drawText(item.url, startX, y, 'F1', 8, accent); y -= 11; }
                if (item.description) { drawWrapped(item.description, startX, colW, 'F1', 10, C_BLACK, 13); }
                for (const b of (item.bullets || [])) { if (b?.trim()) drawWrapped('• ' + b, startX + 8, colW - 8, 'F1', 10, C_BLACK, 13); }
                y -= 6;
            }
        }
    }
